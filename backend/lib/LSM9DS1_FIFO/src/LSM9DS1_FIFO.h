...
/* by the LSM9DS1_FIFO::Sensor class to read and retrieve individual sensors. */
typedef void (LSM9DS1_FIFO::*lsm9ds1_read_func)(void);
typedef void (LSM9DS1_FIFO::*lsm9ds1_get_event_func)(sensors_event_t *, uint32_t);
typedef void (LSM9DS1_FIFO::*lsm9ds1_get_sensor_func)(sensor_t *);

class LSM9DS1_FIFO
{
public:
  LSM9DS1_FIFO(int32_t sensorID = 0);
  LSM9DS1_FIFO(TwoWire *wireBus, int32_t sensorID = 0);
  LSM9DS1_FIFO(int8_t xmcs, int8_t gcs, int32_t sensorID = 0);
  LSM9DS1_FIFO(int8_t clk, int8_t miso, int8_t mosi, int8_t xmcs, int8_t gcs, int32_t sensorID = 0);
  ...
  typedef enum
  {
    ...
    LSM9DS1_REGISTER_CTRL_FIFO = 0x2E,
  ...
  public:
    Sensor() {}
    Sensor(const Sensor &copy) : _parent(copy._parent),
                                 _readFunc(copy._readFunc),
                                 _eventFunc(copy._eventFunc),
                                 _sensorFunc(copy._sensorFunc)
    {
    }
    Sensor(LSM9DS1_FIFO *parent, lsm9ds1_read_func readFunc,
           lsm9ds1_get_event_func eventFunc, lsm9ds1_get_sensor_func sensorFunc) : _parent(parent),
                                                                                   _readFunc(readFunc),
